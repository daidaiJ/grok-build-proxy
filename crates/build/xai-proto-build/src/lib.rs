...
impl XaiProtoBuilder {
    ...
    fn emit_rerun_if_changed<'a>(
        ...
        for proto in protos {
            // LOCAL: /dev/stdout and /dev/null are Unix-only; on Windows route
            // both to temp files and feed the dependency file back through the
            // same parsing path below. protoc's make line is
            // "<descriptor_set>: <deps>", so the prefix has to follow the
            // descriptor target. Both temp files are removed after parsing.
            let windows = cfg!(windows);
            let temp_dep_file = windows.then(|| {
                std::env::temp_dir().join(format!(
                    "xai-proto-deps-{}-{}.txt",
                    std::process::id(),
                    proto.file_stem().and_then(|s| s.to_str()).unwrap_or("proto")
                ))
            });
            let temp_descriptor = windows.then(|| {
                std::env::temp_dir().join(format!(
                    "xai-proto-null-{}-{}.bin",
                    std::process::id(),
                    proto.file_stem().and_then(|s| s.to_str()).unwrap_or("proto")
            ...
            let dependency_out: String = match &temp_dep_file {
                Some(path) => format!("--dependency_out={}", path.display()),
                None => "--dependency_out=/dev/stdout".to_string(),
            };
            let descriptor_target: String = match &temp_descriptor {
            ...
            command
                .arg(dependency_out)
                .arg(format!("--descriptor_set_out={descriptor_target}"));
            ...
            let output = if windows {
                // LOCAL: Windows path — the dependency list went to the temp file.
                let path = temp_dep_file
                    .as_ref()
                    .expect("windows dep file must exist when windows is set");
                let contents = fs::read_to_string(path).context("protoc dependency file unreadable")?;
                let _ = fs::remove_file(path);
                if let Some(desc) = &temp_descriptor {
            ...
            let prefix = if windows {
                format!(
                    ...
                    temp_descriptor
                        .as_ref()
                        .expect("windows descriptor file must exist when windows is set")
                        .display()
                )
            } else {
                "/dev/null:".to_string()
            ...
            for line in iter::once(rem).chain(lines) {
                let line = line.trim();
                // LOCAL: Windows make-format escapes backslashes inside paths.
                let line = if windows {
                    line.replace("\\\\", "\\")
                } else {
                    line.to_string()
                ...
                // LOCAL: match Windows backslash include paths too.
