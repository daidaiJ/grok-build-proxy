...
> **状态（2026-09-20）**：T1、T2 已按本文施工完成并提交（`feat/local-stats-modal-i18n`）。
> **月窗按需求移除**，时间窗改为 **5h / day / week** 三档（正好对齐用量面板的三档），
> 模态窗口共 4 个标签页（三个窗口 + 压缩账本）。压缩账本没有丢：独占第 4 个标签页。
> **T3（全仓 i18n 扫尾）未做**——本批只覆盖 `/stats` 自身新增文案。
> 实现记录见 `docs-local/PATCHES.md` 的「/stats 窗口化」一节；下文保留原始调研内容
> （含月窗表述）作为决策留痕，实际落地以下方状态为准。
>
> LOCAL 特性。分支 `feat/local-stats-modal-i18n`，基线 `86a6e1d`（= 当时 `main`）。
...
| 项 | 原计划 | 实际落地 |
|---|---|---|
| 时间窗 | 5h / day / week / **month** | 5h / day / week（**月窗移除**） |
| 模态标签页 | 三窗口 | 三窗口 + **压缩账本**（第 4 页，信息不丢） |
| 参数 | `/stats [5h\|day\|week\|month]` | `/stats [5h\|day\|week]` |
| 极简模式 | 未明确 | **保留文本报表**（极简无浮动窗口） |
| 数据层 | 依赖 T1 产出 | 新增 `Window` 枚举作单一事实来源，参数解析与标签页共用 |
